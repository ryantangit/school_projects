...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Does not match with the vals')
        while(formals != nil):
...
def do_and_form(expressions, env):
    ...
    for x in range(len(expressions)-1):
        if scheme_falsep(scheme_eval(expressions.first, env, True)):
            return False
...
class MuProcedure(Procedure):
    ...
    # BEGIN PROBLEM 16
    def make_call_frame(self,args,envs):
        return envs.make_child_frame(self.formals, args)
    # END PROBLEM 16
