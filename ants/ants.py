...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if insect.is_ant and insect.can_contain(self.ant):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if hasattr(insect, 'is_realqueen') and insect.is_realqueen:
                insect.place = placeholder
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current.entrance != None:
            if current.bees != []:
                if min_range <= range_tracker and max_range >= range_tracker:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting != 0:
            self.digesting -= 1
        elif self.digesting == 0 and random_or_none(self.place.bees) != None:
...
# END Problem 13
    ...
    buffed_ants = []
    is_realqueen = False
    realqueen_exists = False
    implemented = True   # Change to True to view in the GUI
    # END Problem 13

    def __init__(self, armor=1):
        # BEGIN Problem 13
        if not self.realqueen_exists:
            Ant.__init__(self, armor)
            self.is_realqueen = True
            QueenAnt.realqueen_exists = True
        else:
            Ant.__init__(self, armor)
        # END Problem 13
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if not self.is_realqueen:
            self.reduce_armor(self.armor)
        else:
            behind = self.place.exit
            while behind :

                if behind.ant and behind.ant not in self.buffed_ants:
                    self.buffed_ants.append(behind.ant)
                    behind.ant.damage*= 2
                if behind.ant and behind.ant.is_container:
                    if behind.ant.contained_ant != None:
                        if behind.ant.contained_ant not in self.buffed_ants:
                            behind.ant.contained_ant.damage *= 2
                            self.buffed_ants.append(behind.ant.contained_ant)
                behind = behind.exit
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        self.armor -= amount 
        if self.is_realqueen and self.armor <= 0:
            bees_win()
        else:
